...
All numbers below are from the full ~14M-row dataset (`notebooks/07_full_data_analysis.ipynb`), not the 1M-row development sample.
...
1. **Covariate balance.** Standardized mean differences (SMD) on all 12 features, treatment vs. control, to check that randomization held before trusting anything downstream.
2. **ATE.** Difference in means for `visit` and `conversion`, with 95% CIs, plus retrospective minimum detectable effect (MDE) at 80% power.
...
Because assignment isn't perfectly independent of the features, the raw difference-in-means ATE was checked against two adjusted estimators: a linear regression with treatment × feature interactions, and a cross-fitted doubly-robust estimator with an estimated (not assumed-constant) propensity.

| outcome | raw ATE | linear-adjusted | cross-fitted DR |
|---|---|---|---|
| `visit` | 0.01034 | 0.00773 | 0.00735 |
| `conversion` | 0.00115 | 0.00100 | 0.00111 |

For `visit`, both adjustments agree and land ~25-30% below the raw estimate — a gap far too large to be noise (~18 raw standard errors). For `conversion`, the two adjustments disagree with each other (one lower, one matching raw), so there's no convincing evidence of bias there. The honest statement for `visit` is a range: the ATE is somewhere between ~0.0074 (adjusted) and ~0.0103 (raw), i.e. roughly 20-27% relative lift, not a single clean number. Section 4 covers why this range can't be collapsed further with this data.
...
- **`conversion`**: top decile measured ATE ≈ 0.0078-0.0086 (~7x the overall 0.00116). Deciles 1-8 sit near 0.0001-0.0002, several CIs crossing zero.
